...
async function simplePrint(input, ontology) {
    return print(input, "print/simple/query.n3", "print/simple/describe.n3", "print/simple/collect.n3", ontology)
}

async function visualPrint(input, ontology) {
    return print(input, "print/visual/query.n3", "print/visual/describe.n3", "print/visual/collect.n3", ontology)
}

async function print(input, query, describe, collect, ontology) {
    let start = performance.now();

    let n3 = [
        { name: 'input.n3', data: input },
        { name: 'describe_core.n3', path: "print/describe_core.n3" },
        { name: 'describe.n3', path: describe },
        { name: 'collect.n3', path: collect },
        // { name: 'query.n3', path: "print/simple/query.n3" }
    ];
    if (ontology)
        n3.push({ name: 'ontology.n3', path: ontology });
    ...
    let out = await eyeWithConfig({
        ...
        // passOnlyNew: true,
        query: { name: 'query.n3', path: query },
        n3: n3
    });
