...
class Module {
    ...
    constructor(path, reasoner) {
        ...
        this.base = this.base.substring(0, this.base.lastIndexOf("/") + 1);
    }
    ...
    async init() {
        ...
        let path = `${window.location.origin}/${this.resolveFromBase(this.path)}`;
    ...
    async resolvePath(path) {
        let resolved = await (await fetch(this.resolveFromBase(path))).text();
        let name = this.nameFromPath(path);
        return { name: name, data: resolved };
    }

    resolveFromBase(path) {
